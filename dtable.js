...
module.exports = {
  ...
  elements: 
  {
    tabRow: '//div[@role="gridcell" and text() = "Alden"]',
    tableRow: '//div[@role="gridcell" and text() = "james"]',
  ...
  async search(query) 
  {
    ...
    await I.waitForFunction(() => document.querySelectorAll('div[role="row"]').length > 1, { timeout: 5 });
  },
  ...
  async isWordNotInTable(word) 
  {
    try 
    {
      await I.waitForElement(this.elements.tabRow, { timeout: 5 });
    } 
    catch (error) 
    {
  ...
  async isWordInTable(word) 
  {
    try 
    {
      await I.waitForElement(this.elements.tableRow, { timeout: 5 });
    } 
    catch (error) 
    {
      return false;
